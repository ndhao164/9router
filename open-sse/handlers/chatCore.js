...
export function resolveUsageAttribution({ requestAttribution, clientRawRequest, body, provider, model }) {
  const requestedModel = requestAttribution?.requestedModel
    || clientRawRequest?.body?.model
    || body?.model
    || `${provider}/${model}`;
  return {
    requestedModel: String(requestedModel),
    ...(requestAttribution?.comboName ? { comboName: String(requestAttribution.comboName) } : {}),
  };
}

export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking, requestAttribution }) {
  const { provider, model } = modelInfo;
  const usageAttribution = resolveUsageAttribution({ requestAttribution, clientRawRequest, body, provider, model });
  ...
  if (passthrough) {
    ...
    if (!translatedBody) {
      trackPendingRequest(model, provider, connectionId, false, true, usageAttribution);
  ...
  trackPendingRequest(model, provider, connectionId, true, false, usageAttribution);
  ...
  const streamController = createStreamController({
    onDisconnect: (reason) => {
      trackPendingRequest(model, provider, connectionId, false, false, usageAttribution);
      if (onDisconnect) onDisconnect(reason);
    },
    onError: () => trackPendingRequest(model, provider, connectionId, false, false, usageAttribution),
  ...
  const sharedCtx = { provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, pxpipe: pxpipeSummary, reqTag, log, requestAttribution: usageAttribution };
  const appendLog = (extra) => appendRequestLog({ model, provider, connectionId, ...extra }).catch(() => { });
  const trackDone = () => trackPendingRequest(model, provider, connectionId, false, false, usageAttribution);
